...
class SnakeRadar(object):
    ...
    def init_visions(self,pos): # initialize visions
        eigth_mark = np.array([
        ...
        for i,_it in enumerate(eigth_mark):
            t = _it.tolist()
            t.append(i)
            visions_lis.append(t)
        self.eigth_marker = np.array(visions_lis)
        self.pos_to_edge_coor = np.array(list(self.pos_to_edge()))
            
    def modify_visions(self): # modify visions when snake run
