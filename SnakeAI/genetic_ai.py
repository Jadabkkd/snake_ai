...
class GeneticAI(object):
    def __init__(self):
        ...
        self.initPopFX:dict = {
            ...
            'Uniform': lambda x, y = None : np.random.uniform(low = -1, high = 1, size=(x,y)) if y else np.random.uniform(low = -1, high = 1, size=(x,)),
            'Xavier': lambda x, y = None : self.Xavier_Main(x,y)
        }
        
        # for visualize NN
        self.node_list:list[np.ndarray] = []
        # for roulette wheel
