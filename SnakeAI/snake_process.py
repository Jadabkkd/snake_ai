...
class Snake(object):
    def __init__(self):
        ...
        self.unUsedGrid_temp:list[list] = []
        self.food_coor : list = []
        self.occupied = []
    ...
    def RandomPosition(self,snk_len): #Random position for snake when start or reset
        ...
        for i in range(snk_len):
            ...
            self.drawBox(body_init,self.SnakeColor)
        self.prepare_and_feed(init = True)
        self.food_coor = None
        self.newFoodcoor()
        if self.drawNeuralNet:
            self.draw_node_line()
    ...
    def runSnake(self):
        ...
        snk_head_lis = head_with_move.tolist()
        ...
        if (snk_head_lis in self.unUsedGrid):
            if snk_head_lis[0] == self.food_coor[0] and snk_head_lis[1] == self.food_coor[1]:
                ...
                if self.RunningScore > self.HighScore:
                    ...
                    self.update_stat('High Score: ',self.RunningScore,'max_score')
                if self.RunningScore == 97:
                    ...
                    self.drawBox(self.SnakeBody[-1].coordinate,self.BackgroundColor)
                    self.remove_food()
                    self.newFoodcoor()
                    self.drawBox(self.SnakeBody[1].coordinate,self.SnakeColor)
                    self.drawBox(snk_head_lis,self.SnakeHeadColor)
    ...
    def prepare_and_feed(self,init = False):
        ...
        self.node_list.append(self.one_hot_encode(self.direction))
        if self.drawNeuralNet:
            self.draw_node()